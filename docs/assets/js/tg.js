// Pace.start();
		...
		var nRuns = $('#nruns').val() || 50;
		// console.log('nRuns', nRuns);
		var nTeams = $('#nteams').val() || 2;
		...
		// console.log('weights', weights);
			...
			if (rawData==='') console.log('No Data');
				...
				setTimeout(function() { // this hack  allows the hide to execute and Pace to start
					...
					var hash=[0,0,0,0, Number.POSITIVE_INFINITY];
					var ii=0;
					while (hash[4]>bestSumScore){
						hash = teamGenerator(data.data);
						console.log('sumScore=', hash[4]);
						ii++;
						if (ii>nRuns) {
					...
					if (nRuns>1) {
						...
						var sumEScores = allHashes
								.map( v => v[4]) // keep only 4th column = sumEScores
								;
						// console.log(sumEScores);
						// console.log(Math.min(...sumEScores));
						console.log(sumEScores.reduce( (r,k) => {r[k]=1+r[k]||1; return r;},{})); // frequencies
